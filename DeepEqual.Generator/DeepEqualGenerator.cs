...
internal sealed class DiffDeltaEmitter
{
    ...
    public void EmitForRoot(SourceProductionContext spc, DiffDeltaTarget root, string? hintOverride = null)
    {
        ...
        // Build once for this root; used by registration + per-type emitters
        var reachable = BuildReachableTypeClosure(root);
        ...
        // Static ctor registers all reachable types (diff/delta delegates).
        w.Open("static " + helperClass + "()");
        ...
        // Public root APIs for the root type
        ...
        // Module initializer to force static ctor run
        ...
        // IMPORTANT: Emit the DeltaTrack partial ONLY for the ROOT type (if it has [DeltaTrack]).
        // This guarantees a single '...__DeltaTrack.g.cs' per type, avoiding duplicate hint names.
        ...
        bool generateDelta)
    {
        ...
        // Convenience overloads without exposing ComparisonOptions anywhere
            ...
            if (deltaTracked && !type.IsValueType)
            {
                ...
                w.Open("if (__r is not null && __r.__HasAnyDirty())");
                {
                    ...
                    for (int idx = 0; idx < ordered.Length; idx++)
                    {
                        ...
                        // Arrays or non-list enumerables => replace (policy)
                        ...
                        // Value-like, shallow or reference => set
                        ...
                        // Nested object: try nested delta, fallback to set
