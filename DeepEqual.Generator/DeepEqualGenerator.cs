...
/// <summary>
/// Incremental generator that produces deep-equality helpers for types marked with
/// DeepEqual.Generator.Shared.DeepComparableAttribute.
///
/// Key changes vs. previous version:
/// - Uses ForAttributeWithMetadataName (FAWMN) to find annotated types.
/// - Passes a value-equatable request model through the pipeline (no symbols/syntax).
/// - Rehydrates symbols only at the final output step by combining with Compilation.
/// - Removes cross-run caches: all caching is per-emission and ephemeral.
/// - Emits one file per annotated type.
/// </summary>
...
public sealed class DeepEqualGenerator : IIncrementalGenerator
{
        ...
        var requests =
            context.SyntaxProvider.ForAttributeWithMetadataName(
                ...
                transform: static (gasc, ct) =>
                {
                    ...
                    if (cycleArg is { } b) cycleTracking = b;
    ...
    /// <summary>
    /// Per-emission worker. All heavy lifting + local caches are scoped to one output emission.
    /// No cross-run state is retained.
    /// </summary>
    private sealed class Emitter
    {
        ...
        private const string DeepComparableAttributeName = DeepComparableAttributeMetadataName;
        ...
        public void EmitForRoot(SourceProductionContext spc, Target root, string? hintOverride = null)
        {
            ...
            var hintName = hintOverride ?? SanitizeFileName(rootFqn + "_DeepEqual.g.cs");
