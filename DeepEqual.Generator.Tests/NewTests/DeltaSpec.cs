...
public class DeltaSpec
{
    ...
    private static int ProbeMetaMemberIndex()
    {
        ...
        var dict = doc.Operations.FirstOrDefault(op =>
            op.Kind is DeltaKind.DictSet or DeltaKind.DictRemove or DeltaKind.DictNested);
    ...
    {
        S_Address? left = null, right = new() { Street = "S", City = "C" };
    ...
    public void List_ReadOnly_Typed_Property_Falls_Back_To_SetMember()
    {
        ...
        Assert.DoesNotContain(doc.Operations,
            o => o.Kind is DeltaKind.SeqReplaceAt or DeltaKind.SeqAddAt or DeltaKind.SeqRemoveAt);
    ...
    public void Dict_Value_Change_Emits_DictSet_And_Applies()
    {
        ...
        b.Meta!["who"] = "z";
        var doc = S_OrderDeepOps.ComputeDelta(a, b);
        Assert.Contains(doc.Operations,
            op => op.Kind == DeltaKind.DictSet && (string)op.Key! == "who" && (string?)op.Value == "z");
        var target = SpecFactories.Clone(a);
        S_OrderDeepOps.ApplyDelta(ref target, doc);
        Assert.True(S_OrderDeepEqual.AreDeepEqual(b, target));
    }
    ...
    public void Dict_Add_And_Remove_Emit_Ops_And_Apply()
    {
        ...
        Assert.Contains(doc.Operations,
            op => op.Kind == DeltaKind.DictSet && (string)op.Key! == "new" && (string?)op.Value == "v");
        var target = SpecFactories.Clone(a);
        S_OrderDeepOps.ApplyDelta(ref target, doc);
        Assert.True(S_OrderDeepEqual.AreDeepEqual(b, target));
    }
    ...
    {
        var a = new S_PolyDictHost
            { Pets = new Dictionary<string, IAnimal> { ["d"] = new S_Dog { Name = "f", Bones = 1 } } };
        var b = new S_PolyDictHost
            { Pets = new Dictionary<string, IAnimal> { ["d"] = new S_Dog { Name = "f", Bones = 2 } } };
    ...
    {
        var a = new S_CaseDictHost
            { Meta = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase) { ["who"] = "me" } };
        var b = new S_CaseDictHost
            { Meta = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase) { ["WHO"] = "you" } };
        var doc = S_CaseDictHostDeepOps.ComputeDelta(a, b);
        Assert.Contains(doc.Operations,
            op => op.Kind == DeltaKind.DictSet && ((string)op.Key!).Equals("WHO", StringComparison.OrdinalIgnoreCase));
        var target = new S_CaseDictHost
            { Meta = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase) { ["who"] = "me" } };
    ...
    public void Arrays_Fallback_To_SetMember()
    {
        ...
        var doc = S_WithArrayDeepOps.ComputeDelta(a, b);
        Assert.DoesNotContain(doc.Operations,
            op => op.Kind is DeltaKind.SeqAddAt or DeltaKind.SeqRemoveAt or DeltaKind.SeqReplaceAt);
    ...
    public void DeltaShallow_On_Collection_Forces_SetMember()
    {
        ...
        Assert.DoesNotContain(doc.Operations,
            o => o.Kind is DeltaKind.SeqAddAt or DeltaKind.SeqRemoveAt or DeltaKind.SeqReplaceAt or DeltaKind.DictSet
                or DeltaKind.DictRemove or DeltaKind.DictNested);
        Assert.Contains(doc.Operations, o => o.Kind == DeltaKind.SetMember);
    ...
    {
        var a = new S_SkipCollectionWrap
        {
            Items = SpecFactories.MakeItems(("A", 1), ("B", 2)), Meta = new Dictionary<string, string> { ["x"] = "1" }
        };
        var b = new S_SkipCollectionWrap
        {
            Items = SpecFactories.MakeItems(("A", 1), ("B", 9)), Meta = new Dictionary<string, string> { ["x"] = "2" }
        };
        var doc = S_SkipCollectionWrapDeepOps.ComputeDelta(a, b);
        Assert.True(doc.IsEmpty);
        var target = new S_SkipCollectionWrap
        {
            Items = SpecFactories.MakeItems(("A", 1), ("B", 2)), Meta = new Dictionary<string, string> { ["x"] = "1" }
        };
    ...
    {
        var a = new S_Order
            { Items = new List<S_OrderItem>(), Meta = new Dictionary<string, string>(), Customer = null };
        var b = new S_Order
            { Items = new List<S_OrderItem>(), Meta = new Dictionary<string, string>(), Customer = null };
        var doc = S_OrderDeepOps.ComputeDelta(a, b);
        Assert.True(doc.IsEmpty);
    ...
    public void Bounds_Safety_Invalid_Seq_Index_Throws()
    {
        ...
        var bad = new DeltaDocument();
        var bw = new DeltaWriter(bad);
    ...
    public void Unknown_Op_Kinds_Are_Ignored_Safely()
    {
        ...
        var bogusOp = Activator.CreateInstance(typeof(DeltaOp), anyIdx, (DeltaKind)999, -1, null, "x", null)!;
        ops.Add(bogusOp);
    ...
    {
        return new Dictionary<string, string>(xs.ToDictionary(p => p.k, p => p.v));
    }

    private static IReadOnlyDictionary<string, S_Address> RO(params (string k, S_Address v)[] xs)
    {
        return new Dictionary<string, S_Address>(xs.ToDictionary(p => p.k, p => p.v));
    }

    private static IReadOnlyDictionary<string, string> ROD(params (string k, string v)[] xs)
    {
        return new ReadOnlyDictionary<string, string>(
            xs.ToDictionary(p => p.k, p => p.v));
    }
    ...
    public void IReadOnlyDictionary_Value_Change_Emits_DictSet_And_Applies_By_Clone()
    {
        ...
        Assert.Contains(doc.Operations,
            o => o.Kind == DeltaKind.DictSet && (string)o.Key! == "who" && (string?)o.Value == "you");
    ...
    public void IReadOnlyDictionary_Add_Remove_Emit_Granular_Ops_And_Apply()
    {
        ...
        Assert.Contains(doc.Operations,
            o => o.Kind == DeltaKind.DictSet && (string)o.Key! == "c" && (string?)o.Value == "3");
        Assert.DoesNotContain(doc.Operations, o => o.Kind == DeltaKind.SetMember);
    ...
    public void IReadOnlyDictionary_Nested_Object_Value_Emits_DictNested_And_Mutates_Value_Instance()
    {
        ...
        Assert.DoesNotContain(doc.Operations,
            o => o is { Kind: DeltaKind.SetMember, Value: IReadOnlyDictionary<string, S_Address> });
